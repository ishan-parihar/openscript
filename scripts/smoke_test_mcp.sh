...
# 4a. tools/list — verify all 83 tools are registered
...
echo "Tool count: $TOOL_COUNT (expected 83)"
if [ "$TOOL_COUNT" -ne 83 ]; then
    echo "  ✗ Expected 83 tools, got $TOOL_COUNT"
    exit 1
fi

# Verify key tools are present
for tool in "transcribe" "reelize.timeline" "tts.generate" "hf.lint" "hf.validate" "hf.snapshot" "hf.render" "hf.classify" "composition.render" "verify.render" "verify.production" "llm.complete" "vision.analyze_clip" "vision.score_clip" "system.config.get" "system.config.set" "director.run"; do
