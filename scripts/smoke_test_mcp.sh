...
# 4a. tools/list — verify all 82 tools are registered
...
echo "Tool count: $TOOL_COUNT (expected 82)"
if [ "$TOOL_COUNT" -ne 82 ]; then
    echo "  ✗ Expected 82 tools, got $TOOL_COUNT"
    exit 1
fi

# Verify key tools are present
for tool in "transcribe" "reelize.timeline" "tts.generate" "hf.lint" "hf.validate" "hf.snapshot" "hf.render" "hf.classify" "composition.render" "verify.render" "verify.production" "llm.complete" "vision.analyze_clip" "vision.score_clip" "system.config.get" "system.config.set"; do
