...
# 4a. tools/list — verify all 113 tools are registered
...
echo "Tool count: $TOOL_COUNT (expected 113)"
if [ "$TOOL_COUNT" -ne 113 ]; then
    echo "  ✗ Expected 113 tools, got $TOOL_COUNT"
    exit 1
fi

# Verify key tools are present
for tool in "transcribe" "reelize" "reelize.brief" "reelize.direct" "tts.generate" "hf.lint" "hf.validate" "hf.snapshot" "hf.render" "hf.classify" "composition.render" "verify.render" "verify.production" "llm.complete" "vision.analyze_clip" "vision.score_clip" "system.config.get" "system.config.set" "director.run" "director.format" "script.format.validate" "system.doctor" "segment.analyze" "broll.probe" "asset.library.status" "asset.ingest" "asset.probe" "asset.rate" "asset.import" "asset.search" "voice.design"; do
