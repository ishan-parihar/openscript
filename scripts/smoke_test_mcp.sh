...
# 4a. tools/list — verify all 77 tools are registered
...
echo "Tool count: $TOOL_COUNT (expected 77)"
if [ "$TOOL_COUNT" -ne 77 ]; then
    echo "  ✗ Expected 77 tools, got $TOOL_COUNT"
    exit 1
fi

# Verify key tools are present
for tool in "transcribe" "reelize.timeline" "tts.generate" "hf.lint" "hf.validate" "hf.snapshot" "hf.render" "hf.classify" "composition.render" "verify.render" "verify.production"; do
