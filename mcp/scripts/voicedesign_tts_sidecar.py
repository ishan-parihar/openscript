...
PROTOCOL (mirrors audio8/gepard sidecars):
  ...
  → {"op":"synth","instruct":"<personality>. <emotion/tone for this line>",
     "text":"Scene line to synthesize directly with Qwen3 VoiceDesign",
     "output_path":"/tmp/scene_1.wav","language":"english",
     "seed":42,"temperature":0.9,"top_k":50}
  ← (same response shape as design — synth IS design: Qwen3 generates the
     line in the voice described by instruct; used by script.to_video so
     character voices are never re-cloned through a separate engine)
...
def _dispatch(req) -> dict:
    ...
    # `synth` is an alias of `design`: BOTH synthesize `text` directly with the
    # Qwen3 VoiceDesign model in the voice described by `instruct` (personality
    # + per-line emotion/tone). The script.to_video routing layer calls `synth`
    # per scene so character voices are generated BY the voice-design model at
    # synthesis time — never re-cloned through a separate cloning engine.
    if op in ("design", "synth"):
