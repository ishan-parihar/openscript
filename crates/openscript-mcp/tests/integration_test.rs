...
fn test_mcp_tools_list() {
    ...
    // Should have 58 tools (43 original + 5 HyperFrames hf.* tools + 1 composition.render + 3 script.* + 2 background.* + 2 sticker.* + 1 script.to_timeline + 1 script.to_video)
    assert_eq!(
        tools.len(),
        58,
        "Expected 58 MCP tools, got {}",
        tools.len()
    );
