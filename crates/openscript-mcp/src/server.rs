    ...
    Ok(serde_json::json!({
        ...
        "capabilities": {
            ...
            "experimental": {}
                    ...
                    "tool_count": crate::tools::tool_definitions().as_array().map_or(0, |a| a.len())
                },
        "instructions": "OpenScript MCP server — 111 tools for AI-directed video creation.\n\nConfig: ~/.openscript/config.json (feature toggles under features.* gate what setup.sh provisions and what the runtime enables; env override OPENSCRIPT_FEATURE_<CAT>_<NAME>=0|1). Prefer director.run for one-shot creation. Call system.doctor for cold-start readiness. Production KPI v3 hard-fails majority procedural + wrong music; stock fail-closed writes .draft.mp4 unless OPENSCRIPT_ALLOW_PROCEDURAL=1.\n\nCONTENT FORMATS (shape HOW you author a script):\n- director.format {type, topic} → full playbook: speaker blueprint (gender + voice.design instructs), scene structure, pacing, reactions, worked example. Types: presentation | podcast | dialogue | comedy_sketch | romcom | meme_reel | documentary | how_to | listicle | storytime | debate | newsflash | review. Each format has a UNIQUE signature (structure_kind, speaker range, pacing, reactions, music mood) — use the differentiator field to pick correctly (e.g. podcast ≠ dialogue: roundtable+memes vs formal Q&A; documentary ≠ presentation: chaptered long-form vs short explainer; how_to = numbered steps; listicle = ranked countdown; debate = adversarial versus format).\n- Podcast/dialogue/comedy/romcom: alternate MALE + FEMALE speakers every scene (format.alternation=male_female). Design the voices with voice.design (Qwen3 VoiceDesign, no cloning) or character.create + character.design_emotion.\n- script.format.validate → checks the script against its declared format (speaker count, alternation, pacing).\n- CLI: openscript video new --format podcast --topic '...' scaffolds a draft script.\n\nGOLDEN TRAJECTORY (from-scratch video creation):\n1. system.doctor or system.capabilities — probe which subsystems are available\n2. script.parse — validate your script JSON\n3. script.to_video — ONE-CALL: script → MP4 (handles TTS, captions, multi-broll backgrounds, GIPHY stickers, music, rendering)\n\nThe script.to_video orchestrator automatically:\n- Generates Kokoro TTS per scene with Parakeet force-alignment\n- Builds word-highlight captions (4 styles, centered, animated)\n- Downloads UNIQUE Pexels stock footage per scene (multi-broll)\n- Downloads GIPHY sticker overlays per speaker\n- Mixes background music with sidechain ducking\n- Returns timeline_preview for inspection\n\nMANUAL CONTROL (optional, for fine-tuning):\n- script.generate_voices → script.build_captions → background.fetch → script.to_timeline → composition.render\n\nNLE EDITING (existing footage):\n- transcribe → srt.prepare → edl.build → timeline.build → broll.director → timeline.render\n\nV2V — VIDEO TO VIDEO (existing video → reel):\n- transcribe → srt.prepare → srt.to_timeline → segment.analyze → [AGENT generates English keywords] → broll.fetch → music.assign → captions → timeline.render\n- Agentic: reelize.brief → agent decides → timeline.build → timeline.add_segment(s) → captions.generate_ass → broll.director → music.assign → sfx.assign → timeline.validate → timeline.render\n\nA2V — AUDIO TO VIDEO (audio file → reel):\n- ONE-CALL (FINALIZED): transcribe → srt.prepare → broll.auto (broll coverage loop + sticker.auto agentic GIPHY stickers + captions.generate_ass, all in one call) → music.assign → timeline.render\n- Agentic (draft → relevance-validation → fetch): transcribe → srt.prepare → segment.analyze → broll.keywords → broll.validate_keywords → broll.fetch → music.assign → captions → timeline.render\n- Gap healing: timeline.validate BROLL_GAP errors → broll.repair (re-runs the agentic keyword→validate→fetch loop for exactly those gaps, with the full timeline as context) → re-validate\n- Agentic: transcribe → srt.prepare → timeline.build → timeline.add_segment(s) → captions.generate_ass → broll.fetch → background.assign → music.assign → sfx.assign → timeline.validate → timeline.render\n\nRENDER ENGINE SELECTION:\n- script.to_video: FFmpeg multilayer (default, fast, no custom animation)\n- timeline.render: FFmpeg filter graph (for NLE editing of existing footage)\n- composition.render: HyperFrames (HTML+GSAP motion graphics) or Remotion (React escape hatch)\n- hf.render: Direct HyperFrames render (for pre-authored HF compositions)\n\nMEDIA SEARCH + DOWNLOAD + ASSIGN:\n- Images: media.search → media.download → overlay.assign\n- GIFs: gif.search → gif.download → overlay.assign\n- B-roll: broll.fetch or background.fetch → broll.assign or timeline.add_track_event\n- Music: music.search (local) or library.search (YouTube-scraped) or stock.search (Pixabay) → music.assign\n- SFX: sfx.search → sfx.assign\n- YouTube clips: youtube.search → youtube.download\n\nMUSIC SOURCES (use the right one):\n- music.search: 20 local stock tracks (fast, but limited selection; warns if synthetic)\n- library.search: 500+ YouTube-scraped tracks (requires library.build first; real licensed music)\n- stock.search: Pixabay API (requires PIXABAY_API_KEY; real music + video)\n\nSTICKERS + OVERLAYS (agentic pipeline, parallel to broll):\n- ONE-CALL: sticker.auto (segment.analyze → sticker.keywords → sticker.validate_keywords → GIPHY → Stickers track)\n- Agentic: sticker.keywords (intent + emphatic draft) → sticker.validate_keywords (GIPHY relevance gate — only approved stickers) → sticker.auto_assign (approved picks download directly; spacing + position cycling)\n- SVG puppet + lip-sync: sticker.load_preset → sticker.render (produces HF HTML)\n- GIPHY transparent stickers: gif.search → gif.download\n- Pexels/Openverse images: media.search → media.download\n- Place any overlay: overlay.assign (position, scale, fade)\n\nHYPERFRAMES (HTML+GSAP animations):\n- hf.classify → hf.lint → hf.validate → hf.snapshot → hf.render\n- composition.render: unified dispatcher (auto-classify → hf-native / interop / legacy-remotion)\n\nTIMELINE MANAGEMENT:\n- timeline.build, timeline.add_segment, timeline.add_track_event\n- timeline.preview (token-efficient inspection), timeline.inspect (deep-dive per layer)\n- timeline.diff (compare two versions), timeline.validate\n\nTTS + VOICE:\n- Kokoro (default, 24kHz, 54 voices): script.generate_voices, tts.generate\n- Voicebox sidecar (faster-qwen3-tts, for Hinglish): voice.profile.list, voice.profile.add\n- Voiceover on timeline: voiceover.generate, tts.commentary\n- Emotion-take presets: voice.profile.add with emotions {id: {ref_audio, ref_text}}; scene emote / tts.generate emotion selects the take (per-line tonality)\n- Character-first (two-part): character.create → character.design_emotion (per-emotion takes) → character.list; scripts reference the character id as speaker voice + scene emote\n- Gender metadata on speakers/profiles (male/female/nonbinary/auto) drives format.alternation=male_female; Kokoro prefixes (af_/am_/bf_/bm_) infer gender automatically\n\nQUALITY CHECKS:\n- verify.audio, verify.captions, verify.render, verify.production\n\nDISCOVERY:\n- system.doctor — cold-start production readiness (ready_for_production + next_actions)\n- system.capabilities — probe available subsystems BEFORE calling other tools\n- help.tool — natural-language tool discovery (e.g. 'add background music')\n\nASSET DEVELOPMENT (user-curated footage library; separate from generation):\n- asset.library.status → asset.ingest (index local footage) → asset.probe (curation pool from Pexels+Pixabay+YouTube with thumbnails) → asset.rate (classify relevance/quality) → asset.import (download winners into mcp/assets/user_library) → asset.search (consumption side; generation Tier 1)\n\n30 TOOL FAMILIES: timeline(11), script(7), hf(5), reelze(2), tts(4), music(4), broll(9), srt(3), voice(4), character(4), sfx(3), verify(4), library(3), overlay(2), background(3), asset(6), sticker(7), stock(2), youtube(2), media(2), gif(2), captions(1), transcribe(1), edl(1), render(1), voiceover(1), voices(1), composition(1), system(2), help(1), director(2)."
    }))
}
